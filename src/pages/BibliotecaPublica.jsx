    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, background: '#1a1a1a', border: '1px solid #3f3f46', borderRadius: 12, padding: '14px', zIndex: 100, width: 240, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
          ...
          {hasFilter && <button onClick={() => { onChange({ from: '', to: '' }); setOpen(false); }} style={{ marginTop: 10, width: '100%', background: 'none', border: '1px solid #27272a', color: '#52525b', borderRadius: 8, padding: '5px', fontSize: 11, cursor: 'pointer' }}>Limpiar</button>}
        </div>
      )}
    </div>
  );
