...
export default function CapsuleDigital() {
  useEffect(() => {
    const style = document.createElement("style");
    style.innerHTML = mailerLiteCSS;
    style.id = "mailerlite-css-capsula";
    document.head.appendChild(style);

    window.ml_webform_success_42311315 = function () {
      window.umami?.track('registro-capsula-digital');
      try {
    ...
    script.id = "mailerlite-script-capsula";
    document.body.appendChild(script);

    return () => {
      const css = document.getElementById("mailerlite-css-capsula");
      if (css && css.parentNode) css.parentNode.removeChild(css);
      const s = document.getElementById("mailerlite-script-capsula");
  ...
  return (
    <>
    <Helmet>
      <title>Cápsula Digital × Antiagencia — Email marketing para ecommerce</title>
      <meta name="description" content="Colaboración especial con Cápsula Digital. Todo lo que necesitas saber sobre email marketing para escalar tu tienda online." />
    </Helmet>
      ...
      <div className="flex-1 flex justify-center px-4 py-12 md:py-20">
        <div className="w-full max-w-2xl">
          ...
          <h1 className="text-2xl md:text-4xl font-bold text-center text-gray-900 leading-tight mb-6">
            Si vienes del podcast de Edu, Cápsula Digital, esto es para ti...
          </h1>
