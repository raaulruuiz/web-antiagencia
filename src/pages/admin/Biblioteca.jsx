...
function TokensModal({ onClose }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/biblioteca/pro/tokens`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const create = async () => {
    if (!newToken.trim() || creating) return;
    setCreating(true);
    try {
      const token = await getToken();
      const res = await fetch(`${API_BASE}/biblioteca/pro/tokens`, {
        method: 'POST',
  ...
  const remove = async (id) => {
    try {
      const token = await getToken();
      await fetch(`${API_BASE}/biblioteca/pro/tokens/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }} onClick={onClose}>
      <div className="rounded-xl border border-zinc-700 flex flex-col" style={{ backgroundColor: 'var(--t-surface)', width: '480px', maxHeight: '80vh' }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-zinc-700">
          <h3 className="text-sm font-semibold text-white">Tokens PRO</h3>
          <button onClick={onClose} className="text-zinc-400 hover:text-white text-lg leading-none" style={{ background: 'none', border: 'none', cursor: 'pointer' }}>×</button>
        </div>
        <div className="flex flex-col gap-3 p-5 overflow-y-auto" style={{ flex: 1 }}>
        ...
        <div className="p-5 border-t border-zinc-700 flex flex-col gap-2">
          <span className="text-xs text-zinc-400 font-medium">Nuevo token</span>
          <input value={newLabel} onChange={e => setNewLabel(e.target.value)} placeholder="Etiqueta (ej: Token de Raúl)"
            className="text-xs rounded-lg px-3 py-2 outline-none"
            style={{ background: 'var(--t-surface2)', border: '1px solid var(--t-border)', color: 'var(--t-text)', colorScheme: 'dark' }} />
          <div className="flex gap-2">
            <input value={newToken} onChange={e => setNewToken(e.target.value)} placeholder="Token de acceso"
              onKeyDown={e => e.key === 'Enter' && create()}
              className="text-xs rounded-lg px-3 py-2 outline-none flex-1 font-mono"
              style={{ background: 'var(--t-surface2)', border: '1px solid var(--t-border)', color: 'var(--t-text)', colorScheme: 'dark' }} />
            <button onClick={create} disabled={!newToken.trim() || creating}
              className="text-xs font-semibold text-white rounded-lg px-4 py-2 transition-colors disabled:opacity-50"
              style={{ background: '#3b82f6', border: 'none', cursor: newToken.trim() ? 'pointer' : 'not-allowed' }}
              onMouseEnter={e => { if (newToken.trim()) e.currentTarget.style.background = '#2563eb'; }}
              onMouseLeave={e => e.currentTarget.style.background = '#3b82f6'}>
              {creating ? '…' : 'Crear'}
            </button>
          </div>
...
function InstallExtensionModal({ onClose }) {
  ...
  const chromeSteps = [
    <><a href="/antiagencia-extension.zip" download className="underline text-white hover:text-zinc-300">Descarga la extensión</a> y descomprime el ZIP.</>,
  ...
  const firefoxSteps = [
    <><a href="/antiagencia-extension-firefox.zip" download className="underline text-white hover:text-zinc-300">Descarga la extensión para Firefox</a> y descomprime el ZIP.</>,
    ...
    <>Listo. Haz clic en el icono de la extensión para capturar cualquier página. Nota: en Firefox la extensión temporal se elimina al cerrar el navegador.</>,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70"
      ...
      <div className="relative w-full max-w-md mx-4 border border-zinc-800 rounded-xl p-6" style={{ backgroundColor: '#111' }}>
        ...
        <h2 className="text-base font-semibold mb-1">
          {variant === 'chrome' ? 'Instalar extensión de Biblioteca' : 'Instalar extensión de Biblioteca — Firefox'}
        </h2>
      ...
      <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
        ...
        <div className="hidden md:flex items-center gap-2 overflow-x-auto pb-1" style={{ flexShrink: 0 }}>
          {selecting ? (
            ...
            <>
              <a href="/anti-biblioteca" target="_blank" rel="noopener noreferrer"
                className="text-xs text-zinc-400 hover:text-white border border-zinc-700 hover:border-zinc-500 px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
      ...
      {showFilters && (() => {
        ...
        return (
          <div style={{ marginBottom: 16, background: 'var(--t-surface)', border: '1px solid #27272a', borderRadius: 12, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{ fontSize: 10, color: 'var(--t-border-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
